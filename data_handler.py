...
for filename in glob.glob('C:/Users/Stelios/Desktop/Thesis/experiments/exp3b/Thermal*.hdf5'):
    ...
    for group_key in hf.keys():
        ...
        data_set = hf[group_key]
        data3.append(data_set[()].max())
        data4.append(data_set[()].min())
    hf.close()
##data1.pop()
##data2.pop()
...
plt.plot(timestamps1, data1, label="MCP1 on Object")
plt.plot(timestamps1, data2, label="MCP2 Ambient")
plt.plot(timestamps1, data3, label="Thermal Camera MAX")
plt.plot(timestamps1, data4, label="Thermal Camera MIN")
plt.legend(loc="upper right")

plt.title('Experiment 3B\n\nThermal Camera MIN and MAX Related to MCP sensors')
